...
export function HomePage() {
  ...
  return <div className="px-4 pb-6 md:px-8 lg:px-12">
    ...
    <section className="mt-14 grid gap-4 md:mt-20 md:grid-cols-[1.1fr_.9fr]"><div className="trust-panel premium-trust-panel"><div className="flex items-start justify-between"><div><p className="eyebrow text-lime">La confiance, en clair</p><h2 className="mt-2 max-w-lg text-3xl font-black leading-tight tracking-[-.06em] text-paper md:text-4xl">Le bon choix commence par une <span className="text-lime">bonne rencontre.</span></h2></div><ShieldCheck className="h-9 w-9 text-lime" /></div><p className="mt-7 max-w-xl text-sm leading-relaxed text-paper/65">Majaay rapproche les personnes, les objets et les opportunités qui méritent de circuler. Découvre, échange directement et décide avec les bonnes informations.</p><div className="mt-8 grid grid-cols-3 gap-2 border-t border-white/10 pt-5"><div><b className="text-xl text-lime">01</b><span className="mt-1 block text-[10px] font-bold uppercase tracking-wider text-paper/50">Découvrir</span></div><div><b className="text-xl text-lime">02</b><span className="mt-1 block text-[10px] font-bold uppercase tracking-wider text-paper/50">Échanger</span></div><div><b className="text-xl text-lime">03</b><span className="mt-1 block text-[10px] font-bold uppercase tracking-wider text-paper/50">Conclure</span></div></div></div><button onClick={() => navigate(user ? 'create-listing' : 'login')} className="sell-cta premium-sell-cta"><span className="eyebrow">Pour les vendeurs ambitieux</span><strong>Ton produit mérite<br /><em>son prochain propriétaire.</em></strong><span className="cta-arrow"><Plus /></span></button></section>
  </div>;
}
